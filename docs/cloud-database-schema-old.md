...
本 app 支持多用户，每位用户通过微信 `openid` 独立隔离数据。

**用户私有集合**（查询时必须带 `openid` 过滤，云函数不得跨用户读写）：
`week_plans`、`week_settings`、`preferences`、`meal_logs`、`shopping_list`、`liked_recipes`、`saved_recipes`、`saved_week_plans`

**共享集合**（系统预置数据全局可读，用户数据仅本人可见）：
- `templates`：`openid = "system"` 为系统预置模板，全局可读；用户自定义模板带真实 `openid`，仅本人可见。通过 `isSystem: boolean` 字段区分。
- `recipes`：同上，`isSystem: true` 为系统预置菜谱，全局可读；用户自建菜谱带 `openid`，仅本人可见。

**权限模型**：
- 云端通过 `wx.cloud.callFunction` 自动注入 `openid`，所有云函数均从云端获取 openid，客户端不可传入，防止越权访问。
...
每用户每周一份：记录备餐套数、日期分配方式、各备餐模板归属、BLW 开关、来源收藏计划、确认状态。
...
| N | number | 是 | 本周备餐套数（1–7） |
| templateIds | string[] | 是 | 长度 N，各备餐对应的 template _id（按备餐1/2…顺序） |
| arrangementMode | string | 是 | 日期分配方式：`'consecutive'`（连续）\| `'alternating'`（交替），默认 `'consecutive'` |
| dayBindings | number[] | 是 | 长度 7（周一→周日），值 1–N 表示该天归属的备餐编号，0 表示未分配 |
| blwByMeal | object | 否 | 各餐 BLW 开关，key 为 mealKey，value 为 boolean |
| confirmed | boolean | 否 | 用户是否已确认本周计划 |
| savedPlanId | string\|null | 否 | 若本周计划来自收藏的周计划，指向 `saved_week_plans._id`；否则为 null |
| savedPlanLabel | string\|null | 否 | 冗余存储来源计划的显示名（如「4月7日-4月13日」），用于前端展示，避免二次查询 |
| createdAt | string | 是 | 创建时间 |
| updatedAt | string | 是 | 更新时间 |

> ⚠️ 旧字段 `templateId`（单个）已废弃，由 `templateIds[]` 替代。
...
```json
{
  ...
  "N": 3,
  "templateIds": ["tpl_system_9-11", "tpl_system_9-11", "tpl_system_9-11"],
  "arrangementMode": "consecutive",
  "dayBindings": [1, 1, 2, 2, 2, 3, 3],
  "blwByMeal": {
    "breakfast": false,
    "snack1": false,
    "lunch": false,
...
|------|------|------|------|
| _id | string | 是 | 系统默认 |
| openid | string | 否 | 空或 "system" 表示系统模板；用户自定义模板填真实 openid |
| isSystem | boolean | 是 | `true` = 系统预置（全局可读）；`false` = 用户自定义（仅本人可见） |
...
|------|------|------|------|
| _id | string | 是 | 系统默认 |
| openid | string | 否 | 空或 "system" 为系统菜谱；用户自建菜谱填真实 openid |
| isSystem | boolean | 是 | `true` = 系统预置（全局可读）；`false` = 用户自建（仅本人可见） |
| name | string | 是 | 菜谱名称 |
...
## 7. preferences（宝宝生日/月龄、过敏食材、BLW 偏好）
...
| blwEnabled | boolean | 否 | 是否启用 BLW 模式，默认 false |
| mealCountOverride | number\|null | 否 | 手动覆盖每日餐次数；null 表示按月龄自动计算 |
...
## 9. liked_recipes（宝宝喜欢的菜谱）

用户主动标记某道菜"宝宝喜欢"时写入，与进食记录 meal_logs 独立。用于「我的 → 宝宝喜欢」功能展示与计数。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| _id | string | 是 | 系统默认 |
| openid | string | 是 | 用户标识 |
| recipeId | string | 是 | 菜谱 _id |
| recipeName | string | 是 | 冗余存储菜谱名，避免关联查询 |
| likedAt | string | 是 | 标记时间 ISO 8601 |
...
```json
{
  ...
  "recipeId": "recipe_004",
  "recipeName": "手指胡萝卜",
  "likedAt": "2025-03-03T18:35:00.000Z"
}
```
...
| 索引名 | 字段 | 类型 | 说明 |
|--------|------|------|------|
| openid_1 | openid | 普通 | 查某用户所有喜欢的菜谱 |
| openid_recipeId | openid, recipeId | 唯一 | 防重复标记 |

---

## 10. saved_recipes（收藏的菜谱）

用户主动收藏菜谱以便后续查阅，与喜欢独立。用于「我的 → 收藏的菜谱」功能。
...
| openid | string | 是 | 用户标识 |
| recipeId | string | 是 | 菜谱 _id |
| recipeName | string | 是 | 冗余存储菜谱名 |
| savedAt | string | 是 | 收藏时间 ISO 8601 |

**示例文档：**
```json
{
  "_id": "saved_recipe_xxx",
  "openid": "oABC123xxxx",
  "recipeId": "recipe_001",
  "recipeName": "南瓜粥",
  "savedAt": "2025-03-04T09:00:00.000Z"
}
```
...
| 索引名 | 字段 | 类型 | 说明 |
|--------|------|------|------|
| openid_1 | openid | 普通 | 查某用户所有收藏菜谱 |
| openid_recipeId | openid, recipeId | 唯一 | 防重复收藏 |

---

## 11. saved_week_plans（收藏的周计划快照）

用户确认周计划时可选择收藏，存储完整快照，供后续复用。快照独立保存，不受原始周数据变更影响。用于「我的 → 收藏的周计划」功能。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| _id | string | 是 | 系统默认 |
| openid | string | 是 | 用户标识 |
| label | string | 是 | 显示名，如「4月7日-4月13日」，列表页展示用 |
| weekStartDate | string | 是 | 来源周的周一日期，仅做记录，不用于查询 |
| prepCount | number | 是 | 备餐套数 N |
| arrangementMode | string | 是 | `'consecutive'` \| `'alternating'` |
| dayBindings | number[] | 是 | 长度 7，值 1–N，表示每天归属的备餐编号 |
| items | array | 是 | 各备餐快照，长度 N |
| items[].templateId | string | 是 | 原备餐模板 _id |
| items[].templateName | string | 是 | 备餐名，如「备餐1」 |
| items[].meals | array | 是 | 各餐次快照 |
| items[].meals[].mealKey | string | 是 | 餐次 key |
| items[].meals[].recipeId | string | 否 | 菜谱 _id |
| items[].meals[].recipeName | string | 是 | 菜谱名（冗余） |
| items[].meals[].blw | boolean | 是 | 该餐是否 BLW |
| savedAt | string | 是 | 收藏时间 ISO 8601 |
...
```json
{
  ...
  "savedAt": "2025-04-06T20:00:00.000Z"
}
```
...
| 索引名 | 字段 | 类型 | 说明 |
|--------|------|------|------|
| openid_1 | openid | 普通 | 查某用户所有收藏计划（按 savedAt desc 排序） |
...
| templates | — | openid, openid+ageBand |
| recipes | — | openid, openid+name |
| meal_logs | openid + date + mealKey | openid + weekStartDate |
| preferences | openid | — |
| shopping_list | openid + weekStartDate | — |
| liked_recipes | openid + recipeId | openid |
| saved_recipes | openid + recipeId | openid |
| saved_week_plans | — | openid |
...
1. 打开微信开发者工具 → 云开发 → 数据库。
2. 新建集合：依次创建 `users`、`week_settings`、`week_plans`、`templates`、`recipes`、`meal_logs`、`preferences`、`shopping_list`、`liked_recipes`、`saved_recipes`、`saved_week_plans`。
3. 对每个集合：进入「索引管理」→ 按上表添加对应字段索引（唯一索引勾选「唯一」）。
4. 可选：在 `templates`、`recipes` 中插入 `openid: "system", isSystem: true` 的示例文档，便于联调。
